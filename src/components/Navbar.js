...
function Navbar() {
  ...
  function checkTop() {
    ...
    if (distanceToTop <= 0) {
      component.classList.add("stickyNav");
    }
  }
  ...
  return (
    <div id="navbar" className=" w-full bg-[#0D2427]">
      <div>
      ...
      <div className=" bg-white h-20 w-20 rounded-full fixed top-3    right-10 hidden  max-md:flex items-center justify-center z-50   ">
      ...
      >
        <div className="bg-white flex flex-col absolute right-0 top-0 pt-20 h-screen w-[50vw] items-center ">
          {navigationLinks.map((e, i) => {
            return (
              <Link className="text-black m-6 text-2xl " href={e.href} key={i}>
                {e.name}
              </Link>
            );
          })}
